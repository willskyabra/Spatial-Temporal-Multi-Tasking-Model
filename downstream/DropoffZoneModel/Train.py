...
import itertools
from Model import DropoffZoneClassifier
...
def plot_heatmap(class_accuracies, filename):
    ...
    plt.title("Dropoff Zone Accuracy Heatmap")
...
def tune_hyperparams(x_train_val, y_train_val, epochs, k_folds, num_classes):
    ...
    for lr, batch_size, w_decay in param_combinations:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(x_train_val)):
            ...
            model = DropoffZoneClassifier(x_train_val.shape[1], num_classes).to(device)
...
def train_model():
    ...
    model = DropoffZoneClassifier(x_train.shape[1], num_classes).to(device)
    ...
    plot_heatmap(class_accuracies, 'dropoff_zone_classifier_heatmap.png')

    torch.save(model, "dropoff_zone_classifier.pt")
    torch.save(model, "dropoff_zone_classifier.pth")
