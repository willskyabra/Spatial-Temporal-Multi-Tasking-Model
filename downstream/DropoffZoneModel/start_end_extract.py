...
def preprocess_data(frame, label_encoder=None):
    ...
    numerical_cols = ['pickup_day', 'pickup_month', 'pickup_year', 'pickup_hour',
                      ...
                      'vendor_id', 'store_and_fwd_flag', 'pickup_zone']

    scaler = StandardScaler()
    frame[numerical_cols] = scaler.fit_transform(frame[numerical_cols])

    # Target variable (dropoff_zone)
    if label_encoder is None:
        frame['dropoff_zone_encoded'] = frame['dropoff_zone']
    else:
        existing_labels = frame['dropoff_zone'].isin(label_encoder.classes_)
        frame = frame[existing_labels]
        
        frame['dropoff_zone_encoded'] = label_encoder.transform(frame['dropoff_zone'])

    return frame[numerical_cols].values, frame['dropoff_zone_encoded'].values
